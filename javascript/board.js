...
function renderNoCard(index) {
  ...
  columns[
    index
  ].innerHTML = `<div is-No-card class="no-card class_show">No Task ${text[index]}</div>`;
}
...
function checkCategoryType(category) {
  ...
  switch (category) {
    case "technicalTask":
      (categoryColor = "turquoise"), (text = "Technical Task");
      break;
    case "userStory":
      (categoryColor = "blue"), (text = "User Story");
      break;
    case "bug":
      (categoryColor = "raspberry"), (text = "Bug");
      break;
    case "feature":
      (categoryColor = "yellow"), (text = "Feature");
      break;
    case "refactor":
    ...
    case "Testing":
      ...
      break;
    case "Analysis":
      (categoryColor = "darkCyan"), (text = "Analysis/Research");
      break;
    case "Design":
      (categoryColor = "rose"), (text = "Design");
      break;
    default:
      (color = categoryColor = "grey"), (text = "No Category");
  }
...
function setPriorityImage(columnNumber, id) {
  let imageArray = [
    "../img/icons/empty-icon.svg",
    "../img/icons/urgent-icon.svg",
    "../img/icons/medium-icon.svg",
    "../img/icons/low-icon.svg",
  ];
  ...
  for (let i = 0; i < list.length; i++) {
    if (list[i].length > 0) {
      for (let x = 0; x < list[i].length; x++) {
        ...
        if (
          keySoup.includes(searchValue.toLowerCase()) ||
          list[i][x]["taskID"] == searchValue
        ) {
